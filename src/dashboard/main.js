...
import { initCommandPalette, openPalette, openProjectPicker, openCustomFolderModal } from './components/command-palette.js'
import {
  getConnection,
  saveConnection,
  parseConnectHash,
...
import { createTodoDock } from './components/todo-dock.js'
import { createPushNotifications } from './ui/push-notifications.js'
import { createCommandHistory } from './components/command-history.js'
...
async function bootstrap() {
  ...
  document.getElementById('lbl-agent-btn')?.addEventListener('click', () => {
    import('./components/command-palette.js').then(m => {
  ...
  function createMobileKebab() {
    ...
    document.getElementById('mkp-palette')?.addEventListener('click', () => {
      closeKebab()
      import('./components/command-palette.js').then(m => m.openPalette()).catch(() => {})
    })
  ...
  const { multiviewActive } = (await import('./state/state.js')).getState()
