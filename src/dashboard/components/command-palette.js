...
import { getState, setState, setActiveDirectory, getActiveDirectory } from '../state/state.js'
...
async function rotateToken() {
  ...
  try {
    const { token } = await import('../api/api.js').then(m => m.rotateAuthToken())
    if (!token) {
      toast('Rotation succeeded but no new token returned')
      return
    }
    const state = await import('../state/state.js').then(m => m.getState())
