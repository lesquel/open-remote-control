## Summary

<!-- What changed? Keep this short. -->

## Linked issue

<!-- Required. PRs should only be opened after maintainer approval. -->

Closes #

## Why

<!-- Why is this change needed? What user problem does it solve? -->

## What changed

- 
- 
- 

## Testing

<!-- Paste commands you ran and results. -->

- [ ] `bun run typecheck`
- [ ] `bun test`
- [ ] Other/manual testing:

## Contribution checklist

- [ ] A maintainer approved the linked issue before this PR was opened.
- [ ] This PR is scoped to one issue.
- [ ] I added or updated tests where appropriate.
- [ ] I did not edit `package.json::version`, `src/server/constants.ts::PILOT_VERSION`, or `CHANGELOG.md` unless explicitly asked by the maintainer.
- [ ] I did not add dependencies without explaining why.
- [ ] I did not include AI attribution, generated-by footers, or `Co-Authored-By` lines.
- [ ] I did not include secrets, tokens, private tunnel URLs, or private logs.

## Screenshots / recordings

<!-- For dashboard/UI changes, add before/after screenshots or a short recording if possible. -->

## Breaking changes

<!-- Usually "None". If any behavior/API/config changes, explain clearly. -->

None.
